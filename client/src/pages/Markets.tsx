  ...
  const Markets = () => {
    ...
    const [hasSavedMarkets, setHasSavedMarkets] = useState(false);

    // Kiểm tra xem có dữ liệu đã lưu hay không
    useEffect(() => {
      const checkSavedMarkets = () => {
        const storedData = localStorage.getItem('marketFormDataList');
        if (storedData) {
          try {
            const parsedData = JSON.parse(storedData);
            setHasSavedMarkets(Array.isArray(parsedData) && parsedData.length > 0);
          } catch (error) {
            setHasSavedMarkets(false);
          }
        } else {
          setHasSavedMarkets(false);
        }
      ...
      // Thêm event listener để cập nhật state khi localStorage thay đổi
      window.addEventListener('storage', checkSavedMarkets);

      return () => {
        window.removeEventListener('storage', checkSavedMarkets);
      };
    }, []);
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 mb-6">
                ...
                <div className="mb-6">
                  ...
                  <SavedMarketData onDataChange={setHasSavedMarkets} />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {filteredMarkets.map((market) => {
                    const yesPercentage =
                      (market.yesPool / (market.yesPool + market.noPool)) * 100 || 50;
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
                ...
                {/* Chỉ hiển thị phần "No markets found" nếu không có dữ liệu đã lưu */}
                {!hasSavedMarkets && (
